...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
    ...
    if initialisation == "normal":
        aaa = th.shared(rn.normal(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    else:
        aaa = th.shared(rn.uniform(size=(1, structure[-1])), name="alpha",
                      borrow=True)
    hist_aaa_grad = T.zeros(aaa.shape)  # For AdaGrad.
    ...
    fffs = [s_feature_matrix.T]
    # Forward propagation.
    for W, b in zip(weights, biases):
        fff = T.nnet.sigmoid(th.compile.ops.Rebroadcast((0, False), (1, True))(b)
                           + W.dot(fffs[-1]))
        fffs.append(fff)
        # fffs[1] = fff1 etc.
    # Reward.
    r = aaa.dot(fffs[-1]).reshape((n_states,))
    ...
    updates = []
    aaa_grad = fffs[-1].dot(svf - expected_svf).T
    hist_aaa_grad += aaa_grad**2
    adj_aaa_grad = aaa_grad/(adagrad_epsilon + T.sqrt(hist_aaa_grad))
    updates.append((aaa, aaa + adj_aaa_grad*learning_rate))
